...
for (let i = 1; i <= 1200; i++) { // ~40 rows per page = 30 pages of data!
...
// Generate deep dive paragraphs to pad textual chapters
...
<head>
    ...
    <style>
        ...
        .chapter { page-break-before: always; margin-top: 1cm; }
...
<div class="title-page">
    ...
    ${logoB64 ? \`<img src="\${logoB64}" width="200" style="margin: 40px 0;" />\` : ''}
...
<div class="chapter">
    ...
    ${tatraB64 ? \`<img src="\${tatraB64}" width="500" />\` : ''}
...
<div class="chapter">
    ...
    ${dozerB64 ? \`<img src="\${dozerB64}" width="500" />\` : ''}
...
<div class="chapter">
    ...
    ${dashB64 ? \`<img src="\${dashB64}" width="700" style="border: 2px solid #ccc; box-shadow: 2px 2px 10px rgba(0,0,0,0.1);" />\` : ''}
...
def init_db(): 
    """Initializes the SQLite database and creates the unified telemetry table.""" 
...
def insert_telemetry(payload: dict): 
    """Inserts a validated sensor dictionary payload into the database.""" 
    conn = sqlite3.connect(DB_NAME) 
    cursor = conn.cursor() 
    try: 
        cursor.execute(""" 
...
def fetch_latest(limit: int = 2000): 
    """Retrieves the latest telemetry block records for rolling feature calculation.""" 
    conn = sqlite3.connect(DB_NAME) 
...
def on_connect(client, userdata, flags, rc, properties=None): 
    ...
    print(f"[mqtt_subscriber] Active wildcard subscription set on: {TOPIC}") 

def on_message(client, userdata, msg): 
    try: 
        payload = json.loads(msg.payload.decode("utf-8")) 
    except (json.JSONDecodeError, UnicodeDecodeError) as exc: 
        print(f"[mqtt_subscriber] WARNING: Malformed JSON packet: {exc}") 
        return 
        
    missing = [f for f in REQUIRED_FIELDS if f not in payload] 
    if missing: 
        print(f"[mqtt_subscriber] WARNING: Missing fields {missing}. Dropping frame.") 
        return 
        
    try: 
        db.insert_telemetry(payload) 
    except Exception as exc: 
        print(f"[mqtt_subscriber] Database Transaction Exception: {exc}") 
